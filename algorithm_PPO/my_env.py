...
class Environment:
    ...
    def add_new_vehicles_by_number(self, n):
        ...
        for i in range(n):
            ...
            start_position = [np.random.poisson(20), lane_y, 0]
            while start_position in car_list:
                start_position = [np.random.poisson(20), lane_y, 0]
            car_list.append(start_position)
    ...
    def act_for_training(self, actions):
        ...
        self.renew_channel()
        self.R_V2I()
        next_state = np.concatenate((self.task_size, self.task_fc, self.tk_t_limit, 
                                    self.upd_size, self.upd_fc, self.upd_t_limit, self.ve_v, self.ve_l, self.g_channel))
        return next_state, reward/self.n_Veh, True
    ...
    def get_state(self):
        state = np.concatenate([self.task_size, self.task_fc, self.tk_t_limit, 
                            self.upd_size, self.upd_fc, self.upd_t_limit, self.ve_v, self.ve_l, self.g_channel])
        return state