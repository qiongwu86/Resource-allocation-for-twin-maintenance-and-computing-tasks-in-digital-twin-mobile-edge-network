...
class Vehicle:
    ...
    def task_generate(self):
        ...
        #task_size = np.float16(task_size)
        unit_frequency = 0.25
    ...
    def update_data(self):
        ...
        #data_size = np.float16(data_size)
...
class Environment:
    ...
    def time(self, f_ij):
        for i in range(len(self.vehicles)):
            ...
            t_dt = upd_size*c_i_DT / V2I_rate[i] + (upd_size*c_i_DT / (f_i_DT*1000))*self.n_Veh 
            self.t_TK[i] = t_tk
            self.t_DT[i] = t_dt
    ...
    def act_for_training(self, actions):
        ...
        self.renew_channel()
        self.R_V2I()
        s_next = np.reshape(np.concatenate((self.task_size, self.task_fc, self.tk_t_limit, 
                                            self.upd_size, self.upd_fc, self.upd_t_limit, self.ve_v, self.ve_l, self.g_channel)), [9, self.n_Veh])
        s_next = np.transpose(s_next)
        return s_next, reward
    
    def act_for_testing(self, actions):
        action_temp = actions.copy()
        reward = np.sum(self.reward(action_temp))/self.n_Veh
        self.vehicle_renew_position()
        self.renew_channel()
        self.R_V2I()
        s_next = np.reshape(np.concatenate((self.task_size, self.task_fc, self.tk_t_limit, 
    ...
    def make_new_game(self, n_Veh = 0):
        self.vehicles = []
        if n_Veh > 0:
            self.n_Veh = n_Veh
        ...
        self.renew_channel()
        self.R_V2I()
        s = np.reshape(np.concatenate((self.task_size, self.task_fc, self.tk_t_limit, 
                                       self.upd_size, self.upd_fc, self.upd_t_limit, self.ve_v, self.ve_l, self.g_channel)),[9, self.n_Veh])
        s = np.transpose(s)
        return s